 ...
 * @author koral--
 */
public class GifDrawableCustom implements Animatable, MediaPlayerControl {
    ...
    final InvalidationHandlerCustom mInvalidationHandler;

    private final RenderTaskCustom mRenderTask = new RenderTaskCustom(this);
     ...
     */
    public GifDrawableCustom(@NonNull Resources res, @RawRes @DrawableRes int id) throws NotFoundException, IOException {
     ...
     */
    public GifDrawableCustom(@NonNull AssetManager assets, @NonNull String assetName) throws IOException {
     ...
     */
    public GifDrawableCustom(@NonNull String filePath) throws IOException {
        this(new GifInfoHandle(filePath), null, null, true);
    }

    /**
     * Equivalent to {@code} GifDrawableCustom(file.getPath())}
     *
     * @param file the GIF file
     * @throws IOException          when opening failed
     * @throws NullPointerException if file is null
     */
    public GifDrawableCustom(@NonNull File file) throws IOException {
        this(file.getPath());
    }
     ...
     */
    public GifDrawableCustom(@NonNull InputStream stream) throws IOException {
        this(new GifInfoHandle(stream), null, null, true);
    }

    /**
     * Creates drawable from AssetFileDescriptor.
     * Convenience wrapper for {@link GifDrawableCustom#GifDrawableCustom(FileDescriptor)}
     *
     * @param afd source
     * @throws NullPointerException if afd is null
     * @throws IOException          when opening failed
     */
    public GifDrawableCustom(@NonNull AssetFileDescriptor afd) throws IOException {
     ...
     */
    public GifDrawableCustom(@NonNull FileDescriptor fd) throws IOException {
     ...
     */
    public GifDrawableCustom(@NonNull byte[] bytes) throws IOException {
     ...
     */
    public GifDrawableCustom(@NonNull ByteBuffer buffer) throws IOException {
     ...
     */
    public GifDrawableCustom(@Nullable ContentResolver resolver, @NonNull Uri uri) throws IOException {
     ...
     *
     * @param inputSource                The {@link InputSource} concrete subclass used to construct {@link GifDrawableCustom}.
     ...
     */
    protected GifDrawableCustom(@NonNull InputSource inputSource,
                                @Nullable GifDrawableCustom oldDrawable,
    ...
    GifDrawableCustom(GifInfoHandle gifInfoHandle, final GifDrawableCustom oldDrawable, ScheduledThreadPoolExecutor executor, boolean isRenderingTriggeredOnDraw) {
        ...
        mInvalidationHandler = new InvalidationHandlerCustom(this);
        mRenderTask.doWork();
     ...
     */
    public void reset() {
        mExecutor.execute(new SafeRunnableCustom(this) {
            @Override
            public void doWork() {
    ...
    /**
     * An {@link GifDrawableCustom#GifDrawableCustom(Resources, int)} wrapper but returns null
     ...
     */
    @Nullable
    public static GifDrawableCustom createFromResource(@NonNull Resources res, @RawRes @DrawableRes int resourceId) {
        try {
            return new GifDrawableCustom(res, resourceId);
     ...
     * There is no buffer in GifDrawableCustom, so buffer is assumed to be always full.
     *
     * @return always 100
     */
    @Override
     ...
     * If GifDrawableCustom is constructed from {@link InputStream} -1 is always returned.
     ...
     * Otherwise (if {@link GifDrawableCustomBuilder#setRenderingTriggeredOnDraw(boolean)} was used with <code>true</code>)
    ...
    public boolean setVisible(boolean visible, boolean restart, boolean changed) {
